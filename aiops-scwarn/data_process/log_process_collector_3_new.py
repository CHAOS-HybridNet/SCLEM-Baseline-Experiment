...
def drain_train(input_data):
    ...
    persistence = FilePersistence(f"{s}_drain3.bin")
...
def time_window_process(train_data_path,csv_file,start_time,end_time,minutes):
    ...
    while start_time_datatime <= section_end_time and section_end_time <= end_time_datatime:
        ...
        # print("+++++++++++++++++++++++++")
        # print(type(match_fail))
        # print("+++++++++++++++++++++++++")

        # json_file_path_parent= csv_file + '/match_fail'
        section_start_time_str = section_start_time_str.replace(':', '-').replace(' ', '_')
        json_file_path_parent = os.path.join(csv_file, 'match_fail')
        os.makedirs(json_file_path_parent, exist_ok=True) 
        json_file_path=os.path.join(json_file_path_parent,f'{section_start_time_str}.json')
        with open(json_file_path, mode='w') as json_file:
            json.dump(match_fail,json_file)
...
def get_train_data(train_data_path):
    ...
    train_data_path = output_path + '/' + 'test_log.csv'
