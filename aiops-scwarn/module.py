...
def output_SC_json_and_csv(sc_id, sc_info, train_data, test_data:list, test_zong_score:dict, zong_threshold:dict, test_dim_score, fen_threshold: list):
    ...
    if max_distance != 0:
        ...
        csv_save_dir = 'result_byte_dance_39case/{0}/result_json_and_csv_{1}/result_csv/{1}/'.format(sc_info['service'],sc_id)
        ...
        # promql=[]
        for i in range(len(promql)-1):
            # 获取在线检测的结果
            online_detection_data_dict = dict()
            online_detection_data_dict[promql[i]] = []
            column = promql[i]
            #print(f'promql[i]:{promql[i]}')
            # if column in test_data.index:
            test_data_column=test_data[column]
            '''
            保存每个测试指标的csv文件
            '''
            ...
            #print(f"promql:{promql[i]},test_data_column:{test_data_column[0:10]}")
            for time_stamp, anomaly_score, threshold, origin_value in zip(test_timestamps, test_dim_score[i], fen_threshold[i]['thresholds'], test_data_column[10:]):
                dot_dict = dict()
                # 将检测结果保存为csv文件
   ...
   # 修改位置2
    ...
    result_json_save_name = 'result_byte_dance_39case/{0}/result_json_and_csv_{1}/result_json/'.format(sc_info['service'],sc_id)
    ...
    result_json_save_path = f"result_byte_dance_39case/{sc_info['service']}/result_json_and_csv_{sc_id}/result_json/result_{sc_id}.json"
...
def run_mlstm(train_data: np.ndarray, test_data: np.ndarray, params, sc_id:str, modal:list):
    ...
    print(len(test_data))
    scores, dim_scores = get_prediction_MLSTM(model, test_data, params['seq_len'], modal)
...
def get_average_difference(sc_id:str, results):
    ...
    df1['column1'] = df1.loc[10:, 'MLSTM'].astype('float64')
