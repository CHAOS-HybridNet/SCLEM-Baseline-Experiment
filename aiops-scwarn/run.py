...
def montage_kpi(train_data_path):
    ...
    # # 合并两个列表
    # return kpi_origin
    df_kpi = pd.read_csv(train_data_path + 'train_kpi.csv')
    df_log = pd.read_csv(train_data_path + 'train_log.csv')
    df_kpi.drop(columns=['timestamp'], inplace=True)
    df_merge = pd.concat([df_log, df_kpi], axis=1)
...
def montage_kpi_test(test_data_path):
    df_kpi = pd.read_csv(test_data_path + 'test_kpi.csv')
    df_log = pd.read_csv(test_data_path + 'test_log.csv')
    df_kpi.drop(columns=['timestamp'], inplace=True)
    df_merge = pd.concat([df_log, df_kpi], axis=1)
...
def main(publish_date, prometheus_address, service, sc_id, train_date,task_count, step, timeout, train_duration, detection_duration,predict_interval):
    ...
    for sc_info in all_sc_info:
        ...
        # 修改位置1：修改存放daily和sc的路径
        # train_data_dir="/home/zhengtinghua/shenchao/aiops-scwarn/byte_dance_0219_final/sc" + '/' + service + '/' + sc_id + '/'
        # test_data_dir="/home/zhengtinghua/shenchao/aiops-scwarn/byte_dance_0219_final/daily"+ '/'+service + '/' + sc_id + '/'

        train_data_dir="/home/zhengtinghua/shenchao/aiops-scwarn/bytedance_39case_final/sc" + '/' + service + '/' + sc_id + '/'
        test_data_dir="/home/zhengtinghua/shenchao/aiops-scwarn/bytedance_39case_final/daily"+ '/'+service + '/' + sc_id + '/'

        # train_data_dir =  "../weihua/data_20241019/data_as_SCWarn/new_train" + '/' + service + '/' + '60001/'
        # test_data_dir = "../weihua/data_20241019/data_as_SCWarn/new_test" + '/'+service + '/' + sc_id + '/'
        ...
        print(f"exist? {os.path.exists(test_data_dir)}")
        ...
        # print("kpi_columns, log_columns:",kpi_columns, log_columns)
        #print(test_length)
        # 获取指标数量
        # print([kpi_columns, log_columns])
        # print(f"train_data:{train_data}")
        train_data=torch.nan_to_num(torch.tensor(train_data),nan=0.0)
        print("===============================================")
        ...
        #print(f'test_data_origin:{test_data_origin[:1]}')
        ...
        #     # 可以添加适当的异常处理逻辑
        result_plot(service, test_length-11, sc_id)
        print('end!')
