...
class MLSTM(nn.Module):
    ...
    def forward(self, seq1, seq2):
        ...
        # print(f"input_dim1: {self.input_dim1}, input_dim2: {self.input_dim2}")
        # print(f"seq1.shape before reshape: {seq1.shape}, seq2.shape before reshape: {seq2.shape}")

        #batch_size:64,input_dim1:52,input_dim2:2
        #seq1.numel()=64*16*52 seq1.numel()=64*16*2
        # print(f"befor:seq1.numel():{seq1.numel()},seq2.numel():{seq2.numel()}")

        # print(f"Before reshape: seq1.shape = {seq1.shape}")

        # 确保 seq1 和 seq2 的形状匹配 LSTM 的输入维度
        # lstm_out1, _ = self.lstm1(seq1.reshape(self.batch_size, -1, self.input_dim1))
        # lstm_out1, _ = self.lstm1(seq1.reshape(-1, 16, 52))  # -1 让 PyTorch 自动推断 batch_size

        lstm_out2, _ = self.lstm2(seq2.reshape(self.batch_size, -1, self.input_dim2))

        # print(f"seq1.shape after reshape: {seq1.shape}, seq2.shape before reshape: {seq2.shape}")
        # print(f"after:seq1.numel():{seq1.numel()},seq2.numel():{seq2.numel()}")
...
def predict(model, test_data, seq_len, modal):
    ...
    with torch.no_grad():
        # print(f"predict参数：seq_len:{seq_len},len(test_data):{len(test_data)}")
        for i in range(seq_len, len(test_data)):
            ...
            print(f"seq1 shape: {seq1.shape}, seq2 shape: {seq2.shape} at index {i}")
