...
from SCWarn.util.preprocess import impute_missing, fill_nan
...
def apply_sliding_window(data, seq_len=10, flatten=False):
    ...
    Returns
    -------
    sequences: data after being applied sliding window to
    labels: the ground truth; for example the values from t-w to t are the input so the value at t+1 is the ground truth.
    """
    seq_ls = []
    label_ls = []
    for i in range(seq_len, len(data)):
