...
class SPOT:
    ...
    def plot_d(self,run_results, result_path:str, sc_id:str, with_alarm = True):
        """
        ...
        plt.xlim((0,self.data.size))
        plt.savefig(result_path.format(sc_id))
        plt.close()
        
        return fig, counter, index
...
class dSPOT:
    ...
    def initialize(self, verbose=True):
        ...
        verbose : bool
            (default = True) If True, gives details about the batch initialization
        """
        # 确保 init_data 的长度大于 depth
        if self.init_data.size <= self.depth:
            raise ValueError(f"Invalid depth ({self.depth}) or init_data size ({self.init_data.size}), resulting in non-positive n_init. Ensure init_data is larger than depth.")
        ...
        # 检查 T 是否为空
        if T.size == 0:
            raise ValueError("The array T is empty, check init_data and depth parameters.")

        S = np.sort(T)  # sort T to get the empirical quantile
        self.init_threshold = S[int(0.98 * n_init)]  # t is fixed for the whole algorithm
    ...
    def plot(self,run_results, result_path:str, sc_id:str, with_alarm = True):
        """
        Plot the results given by the run
...
class bidSPOT:
    ...
    def plot(self,run_results, with_alarm = True):
        ...
        ts_fig, = plt.plot(x,self.data,color=air_force_blue,ls='-')
        fig = [ts_fig]
