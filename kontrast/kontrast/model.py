    ...
    def test(self, dataloader: DataLoader):
        ...
        with torch.no_grad():
            for batch_x1, batch_x2, batch_y, batch_id, batch_case_id, batch_case_label in tqdm.tqdm(dataloader):
                ...
                # 找出最小的批次大小，确保所有数组大小一致
                min_size = min(batch_id.shape[0], batch_y.shape[0], batch_case_id.shape[0], batch_case_label.shape[0], loss.shape[0])

                # 对所有数组进行裁剪
                batch_id = batch_id[:min_size]
                batch_y = batch_y[:min_size]
                batch_case_id = batch_case_id[:min_size]
                batch_case_label = batch_case_label[:min_size]
                loss = loss[:min_size]

                batch_result = np.concatenate([batch_id, batch_y, batch_case_id, batch_case_label, loss], axis=-1)

                print(batch_id.shape, batch_y.shape, batch_case_id.shape, batch_case_label.shape, loss.shape)
