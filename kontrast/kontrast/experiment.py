...
class Experiment:
    ...
    def test(self) -> pd.DataFrame:
        """
        Test the model with threshold adjustment based on 'dist'.
        Returns:
            DataFrame
        """

        result = None
        threshold = 0.5  # 仍然使用 0.5 阈值，观察小于阈值的样本

        for i, m in enumerate(self.models):
            cate_result = m.test(self.dataset.get_test_data_loader(i))

            if cate_result is not None:
                print(cate_result.columns)  # 打印 DataFrame 的列名，确认 'dist' 存在
                print(cate_result.head())   # 打印前几行数据
                
                # 反转阈值判断，dist 小于阈值的为正类
                cate_result['preds'] = (cate_result['dist'] < threshold).astype(int)
